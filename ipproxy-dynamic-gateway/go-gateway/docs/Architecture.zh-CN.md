...
Go 版兼容原项目 `areaMapping.csv`。地址映射支持范围和融合池范围是两个概念：

| 供应商 ID | 名称 | 是否融合池 | 是否支持地址映射 |
|-----------|------|------------|------------------|
| 16 | INFATICA | 是 | 是 |
| 17 | NETNUT | 是 | 是 |
| 18 | liang | 否 | 是 |

因此 liang 支持国家、省、城市地址转换，但不会触发融合池专属逻辑。
...
融合池特殊逻辑保持只作用于 INFATICA/NETNUT。例如 sticky session 的 `keeptime > 30min` 时，仍只会优先选择 INFATICA，不会因为 liang 支持地址映射而把 liang 视为融合池。
